...
class TabController(QObject):
    ...
    def __init__(self, form_ctrl: QObject, list_ctrl: RecordListController, record_type: str) -> None:
        super().__init__()
    ...
    def _wire_form_signals(self) -> None:
        ...
        self._form_ctrl.create_requested.connect(lambda p: self.create_requested.emit(rt, p))
        self._form_ctrl.update_requested.connect(lambda p: self.update_requested.emit(rt, p))
        self._form_ctrl.delete_requested.connect(lambda p: self.delete_requested.emit(rt, p))

    def _wire_list_signals(self) -> None:
        rt = self._record_type
        self._list_ctrl.search_requested.connect(lambda q: self.search_requested.emit(rt, q))
        self._list_ctrl.show_all_requested.connect(lambda: self.show_all_requested.emit(rt))
